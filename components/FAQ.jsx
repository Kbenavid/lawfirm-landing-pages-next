...
export default function FAQ({ t }) {
  ...
  const copy = {
    ...
    expandOpen: t?.faq?.expandOpen ?? "–",
    expandClosed: t?.faq?.expandClosed ?? "+",
    cta: {
      heading: t?.faq?.cta?.heading ?? "Still have questions?",
      body:
        t?.faq?.cta?.body ??
        "A quick conversation can bring clarity. Call now or send a message and we’ll respond shortly.",
      callNow: t?.faq?.cta?.callNow ?? "Call Now",
      sendMessage: t?.faq?.cta?.sendMessage ?? "Send a Message",
    },
    questions:
      t?.faq?.questions ??
      [
  ...
  return (
    <section className="mx-auto w-full max-w-screen-2xl px-6 py-16">
            ...
            >
              ...
              >
                ...
                <span className="text-2xl leading-none text-brand-gray">
                  {isOpen ? copy.expandOpen : copy.expandClosed}
                </span>
              </button>
      ...
      <div className="mx-auto mt-10 max-w-3xl rounded-2xl bg-white/80 p-8 shadow-lg ring-1 ring-black/5 backdrop-blur-sm">
        <h3 className="text-xl font-bold text-brand-purple">{copy.cta.heading}</h3>
